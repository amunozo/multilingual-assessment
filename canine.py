    ...
    for i in range(len(tokens)):
        ...
        for j in range(len(tokens[i])):
            ...
            for k in range(len(tokens[i][j])):
                #if k == len(tokens[i][j]) - 1:
                if k == 0:
                    sentence_labels.append(tags[i][j])
                else:
...
def train(
    ...
    if treebank == 'UD_Ancient_Greek-Perseus' or treebank == 'french':
    ...
    #dataset = dataset.rename_columns({"pos_tags": "labels"})
    # Define metric
    ...
    if finetuned == 'not_finetuned':
        for name, param in model.named_parameters():
            if name.startswith("canine"): # CHECK FOR THE NAME OF THE LAYERS!!
...
def predict(treebank, lm, finetuned, pretrained,
        ...
        # Dataset
        #'data/const/single/google/canine-c/UD_Bulgarian-BTB/'
